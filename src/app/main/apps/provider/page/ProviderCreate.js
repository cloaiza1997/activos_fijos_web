...
// Components
import { PROVIDER_PAGE_VIEW, PROVIDER_URL_STORE, PROVIDER_URL_CREATE } from '../ProviderConst';
import ProviderModel from '../model/ProviderModel';

/**
 * @function ProviderCreate
 * @brief Creación de proveedor
 * @date 01/06/2021
...
export default function ProviderCreate() {
	...
	const onStoreProvider = () => {
		...
		const success = ({ provider }) => {
			redirect(`${PROVIDER_PAGE_VIEW}/${provider.id}`);
		};

		const error = () => setLoading(false);

		axios({ url: PROVIDER_URL_STORE, method: 'POST', data: form, success, error });
	};
	...
	useEffect(() => {
		...
		axios({ url: PROVIDER_URL_CREATE, method: 'GET', success });
	}, []);
